...
HTML больше не описывает только документ.

HTML начинает описывать поведение интерфейса.

```html
<button popovertarget="menu"></button>
```

Это уже практически декларативная программа.
...
> «управляй состоянием интерфейса».

Именно здесь стоит показать изменение философии HTML.
