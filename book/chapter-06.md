...
На момент 2026 года поддержка стабильна в:
...
Для старых браузеров используйте graceful degradation:

```css
/* Запасной вариант: без анимации, но работоспособно */
.accordion-panel {
    height: 0;
    overflow: hidden;
    transition: height 0.3s;
...
/* Поддержка браузерами, которые не знают interpolate-size,
   будет просто мгновенное переключение — приемлемо */
...
### Резюме: новая эра CSS-анимаций

Три инструмента — `interpolate-size`, `calc-size()` и `transition-behavior: allow-discrete` — превращают CSS из языка, который "почти умеет анимировать", в полноценную анимационную платформу.

| Что было раньше | Что стало теперь |
|----------------|------------------|
| Хаки с `max-height: 9999px` | `height: auto` с плавной интерполяцией |
| Разрывные анимации с `display: none` | Плавное появление с `@starting-style` |
| JavaScript-вычисления `scrollHeight` | `calc-size(auto, size + Npx)` |
| Зависимость от внешних библиотек | Чистый CSS, встроенный в браузер |

**Ключевой вывод:** Современный CSS позволяет создавать анимации, которые раньше требовали тысяч строк JS-кода. Используйте эти возможности, чтобы ваши интерфейсы были не только красивыми, но и производительными, доступными и поддерживаемыми.

---

Вот доработанный заключительный раздел главы. Я раскрыл архитектурную значимость `<details>` как системного компонента, провел параллели с компонентными фреймворками и показал, почему это меняет подход к веб-разработке.
